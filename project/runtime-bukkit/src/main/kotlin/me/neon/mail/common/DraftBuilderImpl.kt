...
class DraftBuilderImpl(
    ...
    // 玩家名称 、 附件
    private val targets: ConcurrentHashMap<UUID, IMailData> = ConcurrentHashMap()
): IDraftBuilder {
    ...
    override fun changeGlobalModel(): IMailData {
        globalModel = true
        val a = getMailSource<IMailAbstract<*>>().data.createNewInstance()
        targets.clear()
                ...
                if (back == 1) {
                    ...
                    targets.forEach { (key, value) ->
                        ...
                        val new: IMail<*> = if (!value.hasAppendix() && value !is DataTypeEmpty) {
                            EmptyMail(UUID.randomUUID(), sender, key)
                        } else {
    ...
    companion object {
        ...
        fun serialize(builder: ConcurrentHashMap<UUID, IMailData>): ByteArray {
