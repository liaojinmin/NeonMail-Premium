...
object CmdCore {
    ...
    val box = CmdVanillaMenu.command

    @CommandBody(permission = "neonMail.command.germ")
    val germ = CmdGermScreen.command
                    ...
                    ServiceManager.bindCode[sender.uniqueId]?.let {
                        ...
                        if (code == text[0]) {
                            sender.getPlayerData()?.let {  data ->
                                ...
                                (sender as Player).sendLang("玩家-邮箱绑定-成功")
    ...
    val adminBox = subCommand {
        dynamic("种类") {
            ...
            execute<CommandSender> { sender, context, _ ->
                if (sender is Player) {
                    CmdVanillaMenu.openMenu(sender, context["种类"], true)
                }
            }
        }
    }
