...
import me.neon.mail.common.menu.edit.DraftBoxMenu
import me.neon.mail.common.menu.edit.DraftMailEditeMenu
import me.neon.mail.common.menu.impl.ReceiveMenu
import me.neon.mail.common.menu.impl.SenderMenu
...
object CmdBox: ICmd {
    ...
    private fun openMenu(player: Player, type: String) {
        ServiceManager.getPlayerData(player.uniqueId)?.let {
            when (type) {
                "senderBox" -> {
                    SenderMenu(player, it).open()
                    return
                }
                "receiveBOX" -> {
                    ReceiveMenu(player, it).open()
                    return
                }
                "editeBox" -> {
