...
class SQLImpl {
    ...
    private val dataSub by lazy {
        if (ServiceManager.sqlConfig.use_type.equals("mysql", ignoreCase = true)) {
            Mysql(ServiceManager.sqlConfig)
        } else Sqlite(ServiceManager.sqlConfig)
    }
                            ...
                            it.executeQuery().get { res ->
                                ...
                                while (res.next()) {
                                    if (res.getString("Key_name") == "idx_sender_target") {
                                        Settings.debug("索引 idx_sender_target 已存在，不再创建...")
                                        create = false
                                        break
                                    }
                                }
                                if (create) {
                                    Settings.debug("索引 idx_sender_target 不已存在，正在创建...")
                            ...
                            it.executeQuery().get { res ->
                                ...
                                while (res.next()) {
                                    if (res.getString("Key_name") == "idx_sender") {
                                        Settings.debug("索引 idx_sender 已存在，不再创建...")
                                        create = false
                                        break
                                    }
                                }
                                if (create) {
                                    Settings.debug("索引 idx_sender 不已存在，正在创建...")
    ...
    fun insertMail(mail: IMail<*>) {
        getConnection {
            this.prepareStatement(
                ...
                it.setString(1, mail.unique.toString())
                ...
                it.setString(9, mail.parseDataName())
                it.setBytes(10, MailRegister.serializeMailData(mail.data))
                // 如果发送者是系统，默认 1
                it.setInt(11, if (mail.sender == MailRegister.console) 1 else 0)
                it.setInt(12, 0)
                it.execute()
            }
        }
            ...
            try {
                this.prepareStatement(
                    ...
                    mails.forEach { mail ->
                        ...
                        it.setString(3, mail.unique.toString())
                        it.addBatch()
                    }
                    it.executeBatch()
                    back.set(true)
            ...
            try {
                ...
                this.prepareStatement(
                    ...
                    edite.forEach { draft ->
                        ...
                        it.setBytes(7, IDraftBuilder.serialize(draft.getTargets()))
                        it.addBatch()
                    }
                    it.executeBatch()
                    commit()
            ...
            this.prepareStatement(
                ...
                it.executeQuery().get { res ->
                    ...
                    while (res.next()) {
                        ...
                        MailRegister.getRegisterMail(type)?.let { im ->
                            ...
                            val data = IDraftBuilder.deserialize(res.getBytes("data"), im.data::class.java)
                            list.add(
    ...
    private fun insertPlayerData(data: IPlayerData) {
        if (dataSub.isActive) {
            getConnection {
                prepareStatement(
    ...
    fun updatePlayerData(data: IPlayerData) {
        if (dataSub.isActive) {
            getConnection {
                prepareStatement(
    ...
    fun selectPlayerData(player: UUID, name: String? = ""): IPlayerData {
        val data = NeonMailAPI.dataImpl.createNewInstance(player, (name ?: Bukkit.getOfflinePlayer(player).name) ?: "")
        if (dataSub.isActive) {
            getConnection {
                this.prepareStatement(
    ...
    fun selectPlayerData(player: Player, callBack: (IPlayerData) -> Unit) {
        return callBack.invoke(selectPlayerData(player.uniqueId, player.name))
    }

    private fun ResultSet.getMailData(): IMail<*> {
        return MailRegister.getRegisterMail(this.getString("type"))?.let { mail ->
            ...
            val data = MailRegister.deserializeMailData(this.getBytes("data"), mail.data::class.java)
            ...
            mailObj.state = MailState.valueOf(this.getString("state"))
