...
val replacements: Map<Regex, (IMail<*>) -> String> by lazy {
    mapOf(
        TYPE to { NeonMailLoader.typeTranslate[it.translateType]?.colored() ?: it.translateType },
        SENDER to { if (it.sender == IMailRegister.console) NeonMailLoader.typeTranslate["系统"]?.colored() ?: "系统" else Bukkit.getOfflinePlayer(it.sender).name!! },
        SENDER_TIME to { format.format(it.senderTimer) },
        GET_TIME to { if (it.collectTimer < 1000) NeonMailLoader.typeTranslate["未提取"]?.colored() ?: "未提取" else format.format(it.collectTimer) },
        TEXT to { it.context },
        STATE to { NeonMailLoader.typeTranslate[it.state.state]?.colored() ?: it.state.state },
