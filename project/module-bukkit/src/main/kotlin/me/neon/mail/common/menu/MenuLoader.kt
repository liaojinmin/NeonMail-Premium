...
object MenuLoader {
    ...
    fun reload() {
        ...
        }.also { info("§7加载 §f菜单配置文件 §7... §8(耗时 $it ms)") }
    }
    ...
    private fun loadMenu(yaml: ConfigFile): MenuData {
        ...
        yaml.getConfigurationSection("icons")?.let {
            it.getKeys(false).forEach { key ->
                val ic = MenuIcon(key.first(),
                    XMaterial.valueOf(it.getString("$key.mats") ?: "PAPER"),
                    it.getInt("$key.model"),
                    it.getString("$key.name")?.colored() ?: " ",
                    it.getStringList("$key.lore").colored(),
                    it.getString("$key.action") ?: ""
                )
                it.getConfigurationSection("subIcon")?.let { cs ->
                    ic.subIcon = MenuIcon(ic.char,
